...
internal static class ChurnCiSeverity
{
    ...
    {
        return $"Churn risk score {r.ChurnRiskScore:F4} (commits={r.TotalCommits}, +{r.LinesAdded}/-{r.LinesRemoved}, totalLines={FormatTotalLines(r.TotalLines)}, authors={r.TotalUniqueAuthors}, coverage={FormatCoverage(r.CoveragePercent)})";
    }

    private static string FormatTotalLines(int? lines) => lines?.ToString(CultureInfo.InvariantCulture) ?? "n/a";
